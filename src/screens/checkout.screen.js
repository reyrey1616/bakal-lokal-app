...
const CheckoutScreen = ({ route }) => {
  ...
  const handleCheckout = async () => {
    const { paymentMethod, deliveryOption, selectedVoucher } = deliveryDetails;
    ...
    if (!paymentMethod) {
      ...
      const updatedCartItem =
        ...
        _cartItems &&
        _cartItems.map(
          ({ buyPrice, quantity, subTotal, variant, dateAdded, product }) => {
            return {
              dateAdded,
              buyPrice,
              quantity,
          ...
          callback: async () => {
            ...
            dispatch(
              updateCustomerInfoStart({
                ...
                callback: async () => {
                  Alert.alert("Bakal Lokal", " Order Successfully Placed!");
                  setTimeout(() => {
            ...
            paymentMethods?.map((data) => {
              return (
                <PaymentMethodItem
                  key={data?._id}
                  selected={
                    deliveryDetails?.paymentMethod === data?.name ? true : false
                  }
                  name={data?.name}
